...
export class TestDetailBaseComponent{
...
confirmationReference:any;
  ...
  "type" : "form"
	...
	pageViewTitle: string = 'TEST_DETAIL';
...
step:workflowInfo.step
       ...
       const methodName: any = "configureFormOnWorkflow";
        let action: Exclude<keyof TestDetailBaseComponent, ' '> = methodName;
...
if (Array.isArray(ele)) {
    ele.map((ele: any) => {
      if (ele.fieldType == 'Date' && data[ele.name]) {
        const formattedDate = new Date(data[ele.name]).toISOString()
  ...
  if (environment.prototype && this.id) {
    ...
    this.testService.getProtoTypingDataById(params).subscribe((res: any) => {
    ...
    const dataSubscription = this.testService.getById (params).subscribe((res: TestBase[]) => {
      this.data = res || {};
      this.formatRawData();
    });
...
workflowInformation.step = workflowDataForNewRec.step || '';
  ...
  const requestedObj = this.generateRequestObj(data);
      ...
      const saveSubscription = this.uploadAttachmentsandSaveData(requestedObj, splittedData).subscribe(res => {
        ...
        const saveSubscription = this.testService[method](requestedObj).subscribe((res:TestBase) => {
          this.onAfterSave(res, data, method, isToastNotNeeded);
        })
        this.subscriptions.push(saveSubscription);
      }
...
uploadAttachmentsandSaveData(data: any, splittedData: any): Observable<any> {
    ...
    if (!this.id) {
       const saveSubscription =  this.testService.create(splittedData.data).subscribe(
        createdData => {
...
const updateSubscription =  this.saveFiles(splittedData).subscribe((dataToUpdate:any)=>{
  ...
  this.testService.update(dataToUpdate.data).subscribe(
   res => {
     subject$.next(res);
     subject$.complete();
   },
...
saveFiles(splittedData: any) {
	...
	editFormula(config:any,fieldname:any){
     ...
     if(fieldConfig.formulaMethod){
       let methodName: Exclude<keyof TestDetailBaseComponent, ' '> = fieldConfig.formulaMethod
