...
import { TestApiConstants } from '@baseapp/test/test/test.api-constants';
...
export class TestListBaseComponent{
...
localStorageStateKey = "test-list";
  ...
  if (formControl.controls[parent] instanceof FormGroup){
    ...
    if (hasDates.length > 0) {
      hasDates.forEach((f: any) => {
        ...
        if (value && Array.isArray(value)) {
           let filter1 = value[0].toISOString();
           let filter2 = value[1] ? value[1].toISOString() : value[1];
          let val = { lLimit: new Date(filter1).getTime(), uLimit: filter2 ? new Date(filter2).getTime(): filter2, type: "Date" }
	...
	if (this.tableConfig.detailPage?.url) {
      const value: any = "parentId";
       let property: Exclude<keyof TestListBaseComponent, ''> = value;
       const methodName: any = "onUpdateChild";
       let action: Exclude<keyof TestListBaseComponent, ''> = methodName;
      ...
      if (hasDates.length > 0) {
        hasDates.forEach((f: any) => {
          ...
          if (dateVal && Array.isArray(dateVal)) {
             let filter1 = dateVal[0].toISOString();
           let filter2 = dateVal[1] ? dateVal[1].toISOString() : dateVal[1];
          let val = { lLimit: new Date(filter1).getTime(), uLimit: filter2 ? new Date(filter2).getTime(): filter2, type: "Date" }
 ...
 updateActions() {
  ...
  this.tableSearchFieldConfig= this.appUtilBaseService.getControlsFromFormConfig(this.tableSearchConfig)
    ...
    if (environment.prototype) {
      gridSubscription = this.testService.getProtoTypingData().subscribe((data: any) => {
  ...
  this.onRefresh();
	...
	if (this.isChildPage && this[property]) {
		...
		this.router.navigate(['../testdetail'], { relativeTo: this.activatedRoute});
	}
}
	getGridConfig() {
    const self = this
    return {
      ...
      ajaxUrl: TestApiConstants.getDatatableData,
      select: true,
  ...
  if (this.isChildPage) {
    ...
    if (this.selectedValues.length > 0) {
      ...
      this.confirmationService.confirm({
        ...
        accept: () => {
          const deleteSubscription = this.testService.delete(requestedParams).subscribe((res: any) => {
